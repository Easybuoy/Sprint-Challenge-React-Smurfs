...
- [x] Explain the differences between `client-side routing` and `server-side routing`.
      Server-side routing is when all resources are being fetched from the server for every route, whilst client-side routing is when
      a the site files are requested only once from the server and javascript takes care of the rest.
